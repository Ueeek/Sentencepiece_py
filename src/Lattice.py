...
class Lattice:
    ...
    def populate_nodes(self, pieces: dict, Trie):
        ...
        UNK_PENA=10
        UNK_score= min(pieces.values()) - UNK_PENA
        ...
        for pos in range(self.size+1):
            for rnode_id in self.begin_nodes_[pos]:
                for lnode_id in self.end_nodes_[pos]:
                    forward_accm[rnode_id] = LogSumExp(
                        forward_accm[rnode_id], self.nodes[lnode_id].score+forward_accm[lnode_id], lnode_id == self.end_nodes_[pos][0])

        for pos in reversed(range(self.size+1)):
            for lnode_id in self.end_nodes_[pos]:
                for rnode_id in self.begin_nodes_[pos]:
                    backward_accm[lnode_id] = LogSumExp(
                        backward_accm[lnode_id], self.nodes[rnode_id].score+backward_accm[rnode_id], rnode_id == self.begin_nodes_[pos][0])
    ...
    def Viterbi(self, ret_piece=False):
        ...
        for pos in range(self.size+1):
            for rnode in self.begin_nodes_[pos]:
                ...
                for lnode in self.end_nodes_[pos]:
                    score = self.nodes[lnode].backtrace_score + \
                        self.nodes[rnode].score
                    if best_node_id is None or score > best_score:
                        best_node_id = lnode
                        best_score = score

                assert best_node_id is not None, "faild to faind best path in Viterbi surface:{} rnode:{}".format(
                    self.surface, self.nodes[rnode].piece)
        ...
        assert self.surface == "".join(best_path_surfaces), "surface: {}  viterbi: {}".format(
            self.surface, " ".join(best_path_surfaces))
        if ret_piece:
            return best_path_surfaces
        return best_path_ids

    def NBest(self, nbest_size: int, ret_piece=False) -> list:
        ...
        if nbest_size == 1:
            return self.Viterbi(ret_piece=ret_piece)
        ...
        def make_hypo(fx: int, gx: int, next_hypothesis: int, node_id: int, hypo_id: int) -> tuple:
            """ 
                ...
                hypo_id: id of hypo
            Returns:
                hypo(tuple):はじめ3つはheapqででpriorityのために(fx,gxが同じ時にも順位をつけれるようにuniqueなhypoidを入れる)
            """
            #TODO はじめ3つをtupleにまとめてもheapqしてくれると想う(その方が_,_,_,dictってしなくていいから落雪がある)
            return (-fx, -gx, hypo_id, {"fx": fx, "gx": gx, "next_hypo": next_hypothesis, "node_id": node_id, "hypo_id": hypo_id})
        ...
        eos_hypo = make_hypo(eos_score, eos_score, None,
                             eos_node_id, eos_hypo_id)
        ...
        while Agenda:
            (_, _, _, top_hypo) = heapq.heappop(Agenda)

            # Reach to BOS
            if top_hypo["node_id"] == self.get_bos_nodes():
                tmp_res = []

                (_, _, _, nexHypo) = Hypos[top_hypo["hypo_id"]]
                while nexHypo["next_hypo"] is not None:
                    tmp_res.append(nexHypo["next_hypo"])
                    _, _, _, nexHypo = Hypos[nexHypo["next_hypo"]]
            ...
            for lnode in self.end_nodes_[self.nodes[top_hypo["node_id"]].pos]:
                _, _, _, cur_hypo = Hypos[top_hypo["hypo_id"]]
                cur_gx = cur_hypo["gx"]
                ...
                new_hypo = make_hypo(
                    new_fx, new_gx, new_next, lnode, new_hypo_id)
        ...
        for res in results:
            ...
            tmp_node_id = []
            for h in res[:-1]:
                _, _, _, cur_hypo = Hypos[h]
                node_id = cur_hypo["node_id"]
                tmp.append(self.nodes[node_id].piece)
                tmp_node_id.append(node_id)
            assert self.surface == "".join(map(str, tmp)), "surface {} tmp:{}".format(
                self.surface, " ".join(tmp))
            result_node_ids.append(tmp_node_id)

        if ret_piece:  # node_idではなくpieceでreturn する
            result_node_piece = []
